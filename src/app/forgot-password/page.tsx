...
import { useResendOtpForgotPassword } from "@/services/user/auth/resend-otp-forgot-password";
...
export default function ForgotPassword() {
  ...
  useEffect(() => {
    ...
    if (isErrorForgotPassword) {
      const err = errorForgotPassword as ErrorResponse;
      const errorMessage =
        err.response?.data?.data?.message || "Terjadi kesalahan";
      setIsLoading(false);
      toast.error(errorMessage);
    }
  }, [isStatusForgotPassword]);

  useEffect(() => {
    if (isSuccessResendOtp) {
      toast.success("OTP Berhasil Terkirim");
    }

    if (isErrorResendOtp) {
      const err = errorResendOtp as ErrorResponse;
      const errorMessage =
        err.response?.data?.data?.message || "Terjadi kesalahan";
      setIsLoading(false);
      toast.error(errorMessage);
